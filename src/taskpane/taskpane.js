...
export const callCAIOContentService = async (docId, sectionHeading) => {
...
export async function insertDataTable(data) {
  return Word.run(async (context) => {
    ...
    const tableParagraph = body.insertParagraph("RFP Metadata Table", Word.InsertLocation.start);
...
export async function insertSummary(summary) {
  return Word.run(async (context) => {
    ...
    console.log(summary);
    try {
      const summaryParagraph = body.insertParagraph("RFP Summary", Word.InsertLocation.end);
  ...
  return Word.run(async (context) => {
    ...
    for (let i = 0; i < paragraphs.items.length; i++) {
      ...
      if (paragraph.style === "heading 2" && paragraph.text.trim() === sectionTitle) {
        ...
        const contentParagraph = paragraph.insertParagraph("", Word.InsertLocation.after);
        contentParagraph.insertText(content, Word.InsertLocation.replace);
        contentParagraph.styleBuiltIn = Word.Style.normal;
      }
    }
  });
...
export async function run_content(sectionTitle) {
  return Word.run(async (context) => {
    ...
    if (text != "") {
      const res = await callCAIOContentService(docid, text);
      if (res != null) {
        console.log(res.data);
        var content = res.data.split("\n").join(" ");
        console.log(content);
        insertSectionContent(text, content);
...
export async function run_review(sectionTitle) {
  return Word.run(async (context) => {
    ...
    for (let i = 0; i < paragraphs.items.length; i++) {
      ...
      if (paragraph.style === "heading 2" && paragraph.text.trim() === sectionTitle) {
        console.log("Found the paragraph");
        contentParagraph = paragraph.getNext();
      }
    }
